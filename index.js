...
app.post("/order", async (req, res) => {
  try {
    ...
    if (topic === "payment" || type === "payment") {
      ...
      // 🔁 Si sigue siendo 0, intentar obtenerlo desde la orden
    ...
    // 🆕 🔄 Intentar recuperar payment_id desde la orden asociada si falta
    ...
    // ✅ 4️⃣ Validar si ya existe el mismo payment_id
    if (paymentId) {
      const { data: pagoDuplicado } = await supabase
        .from("pagos")
        .select("id")
      ...
      if (pagoDuplicado?.length > 0) {
        console.log("⚠️ Este payment_id ya fue procesado, se ignora duplicado");
        return res.sendStatus(200);
      }
    }

    // 🟢 5️⃣ Insertar en Supabase (permitiendo varios pagos del mismo libro)
